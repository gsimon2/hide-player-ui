...
Hooks.on('ready', () => {
    ...
    const isPlayerUiOverridden = game.user.isGM === false && (game.settings.get('hide-player-ui', 'hideForAllPlayers') || hiddenPlayersList.includes(playerName));
    ...
    if (playerConfig.hideNavigation.complete || (isPlayerUiOverridden && settings.hideNavigation.complete)) {
        hideElement('navigation');
    } else {
        (playerConfig.hideNavigation.navToggle || (isPlayerUiOverridden && settings.hideNavigation.navToggle)) && hideElement('navToggle');
        (playerConfig.hideNavigation.sceneList || (isPlayerUiOverridden && settings.hideNavigation.sceneList)) && hideElement('sceneList');
        (playerConfig.hideNavigation.bossBar || (isPlayerUiOverridden && settings.hideNavigation.bossBar)) && hideElement('bossBar');
    }
    ...
    if (playerConfig.hideSideBar.complete || (isPlayerUiOverridden && settings.hideSideBar.complete)) {
        hideElement('sidebar');
    } else {
        (playerConfig.hideSideBar.chatLog || (isPlayerUiOverridden && settings.hideSideBar.chatLog)) && hideElement('chatLog');
        (playerConfig.hideSideBar.combatTracker || (isPlayerUiOverridden && settings.hideSideBar.combatTracker)) && hideElement('combatTracker');
        (playerConfig.hideSideBar.actorsDirectory || (isPlayerUiOverridden && settings.hideSideBar.actorsDirectory)) && hideElement('actorsDirectory');
    ...
    if (game.modules.get('custom-hotbar') && game.modules.get('custom-hotbar').active) {
